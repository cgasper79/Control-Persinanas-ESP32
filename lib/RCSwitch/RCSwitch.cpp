...
void RCSwitch::setProtocol(int nProtocol) {
  ...
  //edit: Protocolo 4 para persianas con longitud de impulso de 250 ms y 4 repeticiones
  else if (nProtocol == 4) {
...
/**
 * edit: Adici�n comando de estado Quad. Enviar Q
 */	
void RCSwitch::sendQuadState(char* sCodeWord) {
  for (int nRepeat=0; nRepeat<nRepeatTransmit; nRepeat++) {  
	this->sendSync(); //Transmisi�n Syn-bit aqui se env�a ANTES!
  int i = 0;
    while (sCodeWord[i] != '\0') {
      switch(sCodeWord[i]) {
        case '0':
